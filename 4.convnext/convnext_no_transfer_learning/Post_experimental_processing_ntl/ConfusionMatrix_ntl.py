...
class ConfusionMatrix(object):
    ...
    def __init__(self, num_classes: int, labels: list):
        self.matrix = np.zeros((num_classes, num_classes))  # 初始化行列均=num_classes的值为0的矩阵
        self.num_classes = num_classes
        self.labels = labels

    def update(self, preds, labels):
        for p, t in zip(preds, labels):  # 预测标签和真是标片进行打包处理
            self.matrix[p, t] += 1

    def summary(self):  # 计算accuracy
        sum_TP = 0
...
if __name__ == '__main__':
    ...
    model_weight_path = "../weights/ntl_best_model.pth"
