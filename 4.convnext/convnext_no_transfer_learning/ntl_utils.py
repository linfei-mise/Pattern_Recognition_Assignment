...
def get_params_groups(model: torch.nn.Module, weight_decay: float = 1e-5):
    # 记录optimize要训练的权重参数
    parameter_group_vars = {"decay": {"params": [], "weight_decay": weight_decay},  # 需要进行lr正则的group
                            "no_decay": {"params": [], "weight_decay": 0.}}  # 不需要进行lr正则的group
    ...
    for name, param in model.named_parameters():
        ...
        if len(param.shape) == 1 or name.endswith(".bias"):   # 不需要进行lr正则
            group_name = "no_decay"
        else:
            group_name = "decay"  # 其他的需要进行lr正则
...
def plot_class_preds(net,  # 实例化的模型
                     images_dir: str,  # plot_img_ntl的根目录
                     transform,  # 验证集使用的图像预处理方法
                     num_plot: int = 5,  # TB中展示的图片个数
                     device="gpu"):  # 训练所用设备类型
    if not os.path.exists(images_dir):  # 判断图片路径是否存在
        print("not found {} path, ignore add figure.".format(images_dir))
        return None

    label_path = os.path.join(images_dir, "label.txt")  # 判断label文件是否存在
    ...
    # {"hat": "0"}
    class_indices = dict((v, k) for k, v in hat_class.items())  # 对掉顺序
    ...
    for img_path, class_name in label_info:
        ...
        # preprocessing
        img = transform(img)  # 验证集预处理方法
        images.append(img)
        labels.append(label_index)

    # batching images
    images = torch.stack(images, dim=0).to(device)  # 增加一个bitch维度
    ...
    for i in range(num_imgs):
        ...
        # CHW -> HWC
        npimg = images[i].cpu().numpy().transpose(1, 2, 0)  # 通过索引的形式提取出来再转换为numpy格式再转换

        # 将图像还原至标准化之前
        # mean:[0.485, 0.456, 0.406], std:[0.229, 0.224, 0.225]
        npimg = (npimg * [0.229, 0.224, 0.225] + [0.485, 0.456, 0.406]) * 255
        plt.imshow(npimg.astype('uint8'))  # uint8 图片常用格式
