...
class AlexNet(nn.Module):
    def __init__(self, num_classes=1000, init_weights=False):  # 初始化函数定义网络在正向传播过程中要用到的层结构
        ...
        self.features = nn.Sequential(  # 将以下结构进行打包处理，构成新的结构，减少工作量。features：提取图像特征的结构
    ...
    def forward(self, x):  # 定义正向传播过程
        outputs = []  # 定义空列表
        for name, module in self.features.named_children():  # 循环遍历features层结构，返回层的名称以及结构
            x = module(x)  # 输入的特征矩阵依次通过module结构
            if name in ["0", "3", "6"]:  # 判断名称是否为0，3，6.也就是AlexNet模型中的三个卷积层对应的索引
                outputs.append(x)  # 将卷积层的输出添加到定义的空列表中

        return outputs  # 返回得到的列表，得到这三个卷积层的输出特征矩阵
